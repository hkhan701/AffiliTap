 ...
 */
const callLemonSqueezyAPI = async (
...
const loadLicenseData = async (): Promise<any> => {
  return browserStorage.get("licenseData")
}
...
export const hasBasicSubscription = async (): Promise<boolean> => {
  ...
  return (
    ...
    (meta?.variant_id === VARIANT_BASIC_PLAN_ID ||
      meta?.variant_id === VARIANT_BASIC_PLAN_ID_TEST)
  )
}
...
export const hasProfessionalSubscription = async (): Promise<boolean> => {
  ...
  return (
    (await hasValidLicense()) &&
    (meta?.variant_id === VARIANT_PRO_PLAN_ID ||
      meta?.variant_id === VARIANT_PRO_PLAN_ID_TEST)
  )
}
...
export const activateLicenseKey = async (
  ...
  const instance_name =
    Date.now().toString(36) + Math.random().toString(36).substring(2) // Using UUID for unique instance name
  const data = await callLemonSqueezyAPI("activate", licenseKey, {
    instance_name,
  })
  ...
  if (
    data.meta.product_id === PRODUCT_ID ||
    data.meta.product_id === PRODUCT_ID_TEST
  ) {
...
export const verifyLicenseKey = async (): Promise<boolean> => {
  ...
  const response = await callLemonSqueezyAPI("validate", licenseKey, {
  ...
  if (
    ...
    (response.meta.product_id === PRODUCT_ID ||
      response.meta.product_id === PRODUCT_ID_TEST)
  ) {
